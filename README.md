...
```
https://feiyue.selab.top/cg-ai-solver.user.js?v=220
```
...
2. 点齿轮 / 模型按钮进入**配置页**，填 **API Base URL**（默认 `https://api.deepseek.com`；GPT 代理一般要带 `/v1`，如 `https://aiapis.help/v1`）、**API Key**，然后点 **🔄 刷新模型列表**（调 `<BaseURL>/models`），在**主模型 / 重试强模型下拉**里选即可，无需手敲；列表里没有的可选「其他/自定义」手填。Base URL 可换成任意 OpenAI 兼容服务。

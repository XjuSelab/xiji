...
```
https://feiyue.selab.top/cg-ai-solver.user.js?v=211
```
...
4. **一键开刷全部**：先读作业列表建校验队列，从第一题起串行解所有作业的所有题，自动提交、自动跳下一题。可随时**停止开刷**。
   - **失败纠错**：未满分时读取判题「错误样例」（期望输出 vs 实际输出），**追加到同一对话**喂回模型（**不换模型**）让其据上下文纠正后重新提交；如此累积多版本，**仍不过才升级到更高模型**（重试强模型）。
...
| `test-extract.mjs` | jsdom 离线单测（23 项：多题型提取 / 填空模板 / 失败反馈解析 / 模型计划 / 判分） |
